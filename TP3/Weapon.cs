...
namespace TP3
{
    public class Weapon
    {
        ...
        public Weapon Clone()
        {
            float reloadTime = Type == WeaponType.Explosive ? ReloadTime / 2 : ReloadTime;
            return Clone(reloadTime);
        }
        private Weapon Clone(float reloadTime)
        {
            return new Weapon(Name, MinDamage, MaxDamage, reloadTime, Type);
        }
    }
}